...
class Restore(object):
    ...
    def restore_configuration(self, device, method, abstract, iteration=10,
        ...
        if method == 'checkpoint' or method == 'config_replace':
            ...
            for i in range(1,iteration):
                try:
                    self.replace_checkpoint_file(device=device, file=self.to_url)
                except Exception as e:
                    log.error(e)
                    if i == iteration-1:
                        raise Exception('Unable to load override configuration')
                    else:
    ...
    def create_checkpoint_file(self, device, file):
        ...
        try:
            ...
            log.error(e)
            raise Exception("Issue saving config to {c}".format(c=cfg_str)) from e
        else:
            if 'error' in ret:
