...
def verify_bgp_peer_address(device, peer_address, expected_state=None, peer_address_only=False, neighbor_command=False, 
    ...
    while timeout.iterate():
        try:
            if neighbor_command:
                out = device.parse('show bgp neighbor {addr}'.format(addr=peer_address[0]))
            else:
                out = device.parse('show bgp summary')
        except SchemaEmptyParserError:
            timeout.sleep()
        ...
        for peer in bgp_peer:
            if expected_state:
                if peer['peer-state'] == expected_state:
                    found.append(peer['peer-address'].split('+')[0])
            else:
                found.append(peer['peer-address'].split('+')[0])
        ...
        if peer_address_only:
            if (set(peer_address) == (set(peer_address) & set(found))) and (len(peer_address) == len(found)):
                return True
        else:
            if set(peer_address) == (set(peer_address) & set(found)):
                return True
