...
class TestUnconfigureIpSshStricthostkeycheck(TestCase):
    ...
    def test_unconfigure_ip_ssh_stricthostkeycheck_accept_new(self):
        self.device = Mock()
        self.device.configure.return_value = None
        
        # Call the function with accept_new=True
        result = unconfigure_ip_ssh_stricthostkeycheck(self.device, accept_new=True)
        
        # Verify the configuration command was called
        expected_cmd = 'no ip ssh stricthostkeycheck accept-new'
    ...
    def test_unconfigure_ip_ssh_stricthostkeycheck_accept_new_success(self):
        self.device = Mock()
        self.device.configure.return_value = None
        ...
        try:
            ...
            self.fail(f"unconfigure_ip_ssh_stricthostkeycheck with accept_new=True raised an exception: {e}")
