...
def verify_ignore_startup_config(device):
    ...
    # If ConfigReg not found, try SWITCH_IGNORE_STARTUP_CFG variable (other IE3K devices)
    match = re.search(r"SWITCH_IGNORE_STARTUP_CFG\s*=\s*(?P<switch_ignore>\d+)", output)
    if match:
        return int(match.group("switch_ignore")) == 1

    # If neither found, raise error
    raise ValueError(
        f"Could not find config register value or SWITCH_IGNORE_STARTUP_CFG in the output of '{cmd}' "
        f"on {device.name}"
    )
