...
class TestUnconfigureFlowMonitorOnVlanConfiguration(TestCase):

    def test_unconfigure_flow_monitor_on_vlan_configuration(self):
        device = Mock()
        device.state_machine.current_state = 'enable'

        result = unconfigure_flow_monitor_on_vlan_configuration(
            device,
            '10',
            'data-mon',
            'input',
        ...
        self.assertIn('no ip flow monitor data-mon input', cfg_lines)
