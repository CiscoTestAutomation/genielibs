...
class TestConfigureAaaAccountingConnectionDefaultStartStopGroupTacacsGroup(TestCase):
    ...
    def test_configure_aaa_accounting_connection_default_start_stop_group_tacacs_group(self):
        self.device = Mock()
        configure_aaa_accounting_connection_default_start_stop_group_tacacs_group(self.device, 'TACACS-group')
        self.assertEqual(
            self.device.configure.mock_calls[0].args,
            ('aaa accounting connection default start-stop group tacacs+ group TACACS-group',)
        )
