...
class TestConfigureAaaAuthorizationNetworkDefaultGroup(TestCase):
    ...
    def test_configure_aaa_authorization_network_default_group_local(self):
        self.device = Mock()
        configure_aaa_authorization_network_default_group(self.device, 'local')
        self.assertEqual(
            self.device.configure.mock_calls[0].args,
            ('aaa authorization network default group local',)
        )

    def test_configure_aaa_authorization_network_default_group_radius(self):
        self.device = Mock()
        configure_aaa_authorization_network_default_group(self.device, 'radius')
        self.assertEqual(
            self.device.configure.mock_calls[0].args,
