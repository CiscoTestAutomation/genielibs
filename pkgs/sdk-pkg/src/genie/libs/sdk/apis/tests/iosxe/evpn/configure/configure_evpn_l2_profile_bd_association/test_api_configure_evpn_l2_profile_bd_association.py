...
from genie.libs.sdk.apis.iosxe.evpn.configure import (
    configure_evpn_l2_profile_bd_association,
)


class TestConfigureEvpnL2ProfileBdAssociation(TestCase):

    def test_configure_evpn_l2_profile_bd_association(self):
        device = Mock()
        device.state_machine.current_state = 'enable'

        result = configure_evpn_l2_profile_bd_association(device, 10, 'evpn_va1')
        ...
        self.assertIn('bridge-domain 10', cfg_lines)
        self.assertIn('member evpn-instance profile evpn_va1', cfg_lines)
