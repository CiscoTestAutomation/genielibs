...
from genie.libs.sdk.apis.iosxe.cat9k.c9800.configure import collect_install_log

class TestCollectInstallLog(unittest.TestCase):

    @patch('genie.libs.sdk.apis.iosxe.cat9k.c9800.configure.log')
    @patch('genie.libs.sdk.apis.iosxe.cat9k.c9800.configure.get_default_dir')
    @patch('genie.libs.sdk.apis.iosxe.cat9k.c9800.configure.datetime')
    def test_collect_install_log_success(self, mock_datetime, mock_get_default_dir, mock_log):
        ...
        with patch.object(type(runtime), 'directory', new_callable=PropertyMock) as mock_dir:
            ...
            device.execute.side_effect = [
                None, # show platform software install-manager chassis active r0 operation current detail
                None, # show platform software install-manager chassis active r0 operation history detail
    ...
    @patch('genie.libs.sdk.apis.iosxe.cat9k.c9800.configure.get_default_dir')
    @patch('genie.libs.sdk.apis.iosxe.cat9k.c9800.configure.datetime')
    def test_collect_install_log_no_archive(self, mock_datetime, mock_get_default_dir, mock_log):
        mock_datetime.utcnow.return_value.strftime.return_value = '20250101T000000000'
        mock_get_default_dir.return_value = "flash:"
        with patch.object(type(runtime), 'directory', new_callable=PropertyMock) as mock_dir:
            mock_dir.return_value = "/tmp"
            ...
            device.execute.side_effect = [
                ...
                None, # show platform software install-manager chassis active r0 operation history detail
                None, # show tech-support install | append show_tech_support.txt
                "", # request platform software trace archive - no archive created
            ]

            collect_install_log(device)

            # Only the show_tech file should be copied, not an archive
            device.api.copy_from_device.assert_called_once_with(
                local_path="flash:show_tech_support_20250101T000000.txt", remote_path="/tmp"
            )

    @patch('genie.libs.sdk.apis.iosxe.cat9k.c9800.configure.log')
    @patch('genie.libs.sdk.apis.iosxe.cat9k.c9800.configure.get_default_dir')
    @patch('genie.libs.sdk.apis.iosxe.cat9k.c9800.configure.datetime')
    def test_collect_install_log_custom_timeout(self, mock_datetime, mock_get_default_dir, mock_log):
        mock_datetime.utcnow.return_value.strftime.return_value = '20250101T000000000'
        mock_get_default_dir.return_value = "flash:"
        with patch.object(type(runtime), 'directory', new_callable=PropertyMock) as mock_dir:
            mock_dir.return_value = "/tmp"
            ...
            collect_install_log(device, timeout=1200)

            # Verify show tech and trace archive calls used custom timeout
            execute_calls = device.execute.call_args_list
            # 3rd call: show tech-support install | append ...
            self.assertEqual(execute_calls[2][1].get('timeout'), 1200)
