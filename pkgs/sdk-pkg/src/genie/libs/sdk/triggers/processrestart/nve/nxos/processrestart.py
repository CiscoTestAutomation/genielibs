'''
Implementation for Nve Restart triggers
'''
...
class TriggerProcessCrashRestartNve(TriggerProcessRestart):
    """Restart the running Nve process(es) with linux command "kill -6 <process>",
    expecting process crashes and generates a core."""
    
    __description__ = """Restart the running Nve process(es) with linux command "kill -6 <process>",
    expecting process crashes and generates a core.
...
class TriggerProcessKillRestartNve(TriggerProcessRestart):
    """Restart the running Nve process(es) with Linux command "kill -9 <process>" """

    __description__ = """Restart the running Nve process(es) with Linux command "kill -9 <process>"
