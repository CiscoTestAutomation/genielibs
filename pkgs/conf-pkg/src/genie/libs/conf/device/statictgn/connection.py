...
class GenieTgn(BaseConnection):
    ...
    def calculate_traffic_outage(self, max_outage=3):

        # Convert from seconds to milliseconds for PSAT TGN
        max_outage = (max_outage * 1000)

        # Calculate traffic outage
        # expect1.1> TGNCalculateAbsoluteOutage -maxOutageMs $maxOutageInMs
        result = tcl.eval('TGNCalculateAbsoluteOutage -maxOutageMs {}'.format(max_outage))

        # Process result
        if result == '1':
            log.info("No traffic outage observed on device '{}'".format(self.device.name))
        else:
            raise GenieTgnError("Traffic outage observed on device '{}'".format(self.device.name))


    def poll_traffic(self, timeout=1800, delay=30):

        # timeout value in seconds [default 1800 seconds]
        # delay value in seconds [default 30 seconds]
        # Convert from seconds to milliseconds for PSAT TGN
        delay = delay * 1000

        # Poll until traffic resumes
        # expect1.1> TGNPollUntilTrafficResumes -timeout $maxOutageTime -delayCheckTraffic $delayCheckTraffic
        result = tcl.eval('TGNPollUntilTrafficResumes -timeout {timeout} -delayCheckTraffic {delay}'.format(timeout=timeout, delay=delay))

        # Process result
        if result == '1':
            log.info("Traffic successfully resumed on device '{}'".format(self.device.name))
        else:
            raise GenieTgnError("Traffic not stable on device '{}'".format(self.device.name))
