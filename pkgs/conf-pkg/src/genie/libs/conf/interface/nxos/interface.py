...
class Interface(genie.libs.conf.interface.Interface):
    ...
    def _build_config_interface_submode(self, configurations, \
        ...
        # tunnel_encryption
        if attributes.value('tunnel_encryption'):
            configurations.append_line(
                attributes.format('tunnel-encryption'),
                unconfig_cmd='no tunnel-encryption')
        ...
        if attributes.value('nve_vni'):
                ...
                with configurations.submode_context(attributes.format(cfg_line, force=True)):
                    ...
                    if attributes.value('nve_vni_ir_proto'):
                        configurations.append_line(attributes.format('ingress-replication protocol {nve_vni_ir_proto}'))

                    if attributes.value('nve_vni_ir_peer_ip'):
                            configurations.append_line(
                                attributes.format('peer-ip {nve_vni_ir_peer_ip} '))
                ...
                for key, value in vni_config_dict.items():
                   ...
                   with configurations.submode_context(attributes.format(cfg_line, force=True)):
                      ...
                      if vni_config_dict[key].get('nve_vni_ir', False):
                          if vni_config_dict[key].get('nve_vni_ir_proto',None) == 'static':
                                cfg_line = attributes.format('ingress-replication protocol {}'.format(vni_config_dict[key].get('nve_vni_ir_proto',None)))
                                with configurations.submode_context(attributes.format(cfg_line, force=True)):
                                    if vni_config_dict[key].get('nve_vni_ir_peer_ip',None) is not None:
                                        configurations.append_line(
