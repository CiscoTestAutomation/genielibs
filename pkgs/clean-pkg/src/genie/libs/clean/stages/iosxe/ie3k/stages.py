...
"""
    ...
    def delete_boot_variables(self, steps, device, timeout=30):
        ...
        with steps.start(f"Delete any previously configured boot variables on {device.name}") as step:
            try:
                device.configure('no boot system', timeout=timeout)
            except Exception as e:
                step.failed(f"Failed to delete the boot variables because of {e}")

    def enable_boot_manual(self, steps, device):
        # To enable the boot manual
        with steps.start(f"Enabling the boot manual on {device.name}") as step:
            try:
