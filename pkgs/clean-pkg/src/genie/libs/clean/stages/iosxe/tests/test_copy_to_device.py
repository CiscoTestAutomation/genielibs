...
class VerifyCopyToDevice(unittest.TestCase):
    ...
    def test_do_not_skip_copy_to_device_for_smu(self):
        class MockExecute:
            ...
            def __init__(self, *args, **kwargs):
                self.data = {
                    ...
                    'copy tftp://127.0.0.1//path/smu.bin bootflash:/smu.bin': iter(['Copied file']),
                }
        ...
        self.cls.copy_to_device(
            steps=steps, device=self.device,
            origin=dict(
                files=['/path/cat9k_iosxe.TEST_LATEST_IMAGE_20230606.SSA.bin',
                       '/path/smu.bin'],
                hostname='server1'
            ),
            destination=dict(
        ...
        self.assertEqual("Copying image file /path/smu.bin to device PE1 bootflash:", steps.details[13].name)
