'''IOSXE SD-WAN specific recovery functions'''
...
from genie.libs.clean.recovery.iosxe.sdwan.dialogs import (BreakBootDialog,
                                                           RommonDialog,
                                                           TftpRommonDialog)
...
def recovery_worker(start,
    ...
    def console_breakboot(spawn, break_count, break_char):
        ...
        log.info(
            "Found the console_activity_pattern! "
    ...
    def grub_breakboot(spawn, break_char):
        ...
        log.info(
            "Found the grub_activity_pattern! "
            "Breaking the boot process by sending '{}'".format(break_char))
...
def tftp_device_recovery(spawn,
                         timeout,
                         device,
                         tftp_boot,
                         item,
    ...
    dialog.add_statement(
        Statement(
            pattern=
            r'^(.*?)({hostname}|Router|Switch|ios|switch)(\\(standby\\))?(-stby)?(\\(boot\\))?(>)'
    ...
    dialog.dialog.process(spawn,
                          ...
                          context={
                              ...
                              device.name,
                              'ip':
                              tftp_boot['ip_address'][item],
                              'password':
                              recovery_password,
                              'subnet_mask':
                              tftp_boot['subnet_mask'],
                              'gateway':
                              tftp_boot['gateway'],
                              'image':
                              tftp_boot['image'],
                              'tftp_server':
                              tftp_boot['tftp_server'],
                              'hostname':
                              device.hostname,
                              'device_managed_mode':
                              tftp_boot['device_managed_mode']
                          })
