''' 
Pim Genie Ops Object for IOS - CLI
'''
# super class
...
class Pim(SuperPim):
    ...
    def learn(self):
        ...
        for vrf in sorted(vrf_list):
            ...
            # up_time
            self.add_leaf(cmd='show ip mroute vrf {vrf}'.format(vrf=vrf), 
            			  src=src,
            			  dest=dest,
            			  vrf=vrf_name)
            self.add_leaf(cmd='show ipv6 mroute', 
            			  src=src, 
            			  dest=dest, 
            			  vrf='')
            ...
            for key in bsr_keys:
                self.add_leaf(cmd='show ip pim vrf {vrf} bsr-router'.format(vrf=vrf),
                              src=src + key,
                              dest=dest + key,
                              vrf=vrf_name)

            for key in bsr_keys:
                self.add_leaf(cmd='show ipv6 pim bsr election',
                              src=src + key,
                              dest=dest + key,
                              vrf='')

            # rp_candidate_next_advertisement
            self.add_leaf(cmd='show ipv6 pim bsr candidate-rp',
            ...
            for key in intf_keys:
                self.add_leaf(cmd='show ipv6 pim interface',
                              src=src + key,
                              dest=dest + key,
                              vrf='')
            ...
            for key in nei_keys:
                self.add_leaf(cmd='show ip pim vrf {vrf} neighbor'.format(vrf=vrf),
                              src=src + key,
                              dest=dest + key,
                              vrf=vrf_name)

            for key in nei_keys:
                self.add_leaf(cmd='show ipv6 pim neighbor detail',
                              src=src + key,
                              dest=dest + key,
                              vrf='')
