"""Unit tests for ``genie.libs.ops.platform.iosxe.rev1.platform.Platform``.
"""
import unittest
...
from genie.libs.ops.platform.iosxe.rev1.platform import Platform
...
from genie.libs.parser.iosxe.rv1.show_platform import \
    ShowInventory as ShowInventoryRev1, \
    ShowPlatform as ShowPlatformRev1


class test_platform_rev1_c9610r(unittest.TestCase):
    ...
    def _build_platform(self):
        ...
        f.maker.outputs[ShowInventoryRev1] = \
    ...
    def test_chassis_attributes_learned(self):
        """Sanity: top-level chassis attributes are populated."""
        f = self._build_platform()
        f.learn()
    ...
    def test_lc_hw_revision_routed_to_lc_subtree(self):
        """Line cards from `show module` land on slot['lc'][...].hw_revision."""
        f = self._build_platform()
        f.learn()
    ...
    def test_sup_hw_revision_routed_to_rp_subtree(self):
        """SUPs from `show module` land on slot['rp'][...].hw_revision (the fix)."""
        f = self._build_platform()
        f.learn()
    ...
    def test_sup_hw_revision_not_smuggled_into_lc(self):
        """SUP slots must NOT appear under slot['lc'] (defensive check)."""
        f = self._build_platform()
        f.learn()
    ...
    def test_mod_attribute_cleaned_up(self):
        """The transient `self.mod` work tree must be deleted after learn()."""
        f = self._build_platform()
        f.learn()
        self.assertFalse(
            hasattr(f, 'mod'),
...
class test_platform_rev1_members_svl(unittest.TestCase):
    """Verify that the normalized `members` collection is populated for a
    multi-chassis IOS XE SVL pair. The importer learns Platform Ops with
    `revision="1"`, so the rev1 module is the one that has to expose this
    contract.
    """
    ...
    def _build_platform(self, show_version=None, show_module=None):
        ...
        # Stub the remaining parsers rev1 Platform.learn() touches; they
    ...
    def test_members_populated_for_svl(self):
        ...
        self.assertTrue(hasattr(f, 'members'),
                        "rev1 Platform Ops did not expose `members` collection")
