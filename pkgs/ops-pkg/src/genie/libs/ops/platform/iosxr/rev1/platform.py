...
class Platform(SuperPlatform):
    ...
    def learn(self):
        ...
        # Update chassis_sn if we find a Top Assembly Block section. For NCS5K
        # devices, the serial number in show inventory does not match with admin
        # show diag chassis. Using the Top Assembly Block section instead to
        # match show inventory output.
        self.add_leaf(cmd=AdminShowDiagChassis,
                      src='[top_assembly_block][serial_number]',
                      dest='[chassis_sn]')

        # Fallback options for chassis serial number using show diag details.
        # If any of below matches, it will update the chassis_sn.
        self.add_leaf(cmd='show diag details', revision='1',
                      src='[item][Rack 0][chassis_serial_number]',
                      dest='[chassis_sn]')

        self.add_leaf(cmd='show diag details', revision='1',
                      src='[item][Rack 0][common_blocks][serial_number]',
                      dest='[chassis_sn]')
        ...
        self.add_leaf(cmd='show diag details', revision='1',
                      src='[item][Rack 0][common_blocks][pid]',
                      dest='[chassis]')
