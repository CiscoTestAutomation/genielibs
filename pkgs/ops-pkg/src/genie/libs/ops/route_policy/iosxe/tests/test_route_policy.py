...
class test_route_policy(unittest.TestCase):
    ...
    def test_ignored(self):
        ...
        f.diff_ignore.append('info[test][statements][10][actions][set_route_origin][origin]')

        self.assertNotEqual(f,g)
        # Verify diff now
        diff = f.diff(g)
        sorted_diff = str(diff)
        sorted_result = ('''+diff_ignore: deque(['maker', 'callables', 'device', 'info[test][statements][10][actions][set_route_origin][origin]'])\n-diff_ignore: deque(['maker', 'callables', 'device'])''')
