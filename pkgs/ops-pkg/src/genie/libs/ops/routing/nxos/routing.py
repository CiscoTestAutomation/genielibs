...
class Routing(Base):
    ...
    def learn(self):
        ...
        src_routing_route = '[vrf][(?P<vrf>.*)][address_family][(?P<af>ipv4)]' \
                                  '[routes][(?P<route>.*)]'
        ...
        ##############################################
        ####            Ipv6                ##########
        src_routing_route_v6 = '[vrf][(?P<vrf>.*)][address_family][(?P<af>ipv6)]' \
                                  '[routes][(?P<route>.*)]'
        ...
        for key in req_key:
            self.add_leaf(cmd=ShowIpv6Route,
                          src=src_routing_route_v6 + '[{}]'.format(key),
                          dest=dest_routing_route_v6 + '[{}]'.format(key),
                          vrf='all')
        ...
        self.add_leaf(cmd=ShowIpv6Route,
                      src=src_routing_intf_v6 + '[outgoing_interface]',
                      dest=dest_routing_intf_v6 + '[outgoing_interface]',
                      vrf='all')
        ...
        for key in req_key:
            self.add_leaf(cmd=ShowIpv6Route,
                          src=src_routing_hop_v6 + '[{}]'.format(key),
