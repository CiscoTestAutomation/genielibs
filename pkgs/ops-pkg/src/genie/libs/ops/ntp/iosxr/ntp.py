''' 
NTP Genie Ops Object for IOSXR - CLI.
'''
import copy
...
class Ntp(SuperNtp):
	...
	def learn(self):
		...
		for src_key, dest_key in {'act_freq': 'actual_freq',
		...
		self.add_leaf(cmd=ShowRunningConfigNtp,
					  src='[vrf][(?P<vrf>.*)][address][(?P<address>.*)][type][(?P<type>.*)]',
					  dest='info[vrf][(?P<vrf>.*)][unicast_configuration][address]'
						   '[(?P<address>.*)][type][(?P<type>.*)][type][(?P<type>.*)]')
		...
		self.add_leaf(cmd=ShowRunningConfigNtp,
					  src='[vrf][(?P<vrf>.*)][address][(?P<address>.*)][type][(?P<type>.*)]',
					  dest='info[vrf][(?P<vrf>.*)][unicast_configuration][address]'
						   '[(?P<address>.*)][type][(?P<type>.*)][vrf][(?P<vrf>.*)]')
					...
					for i, vrf in enumerate(self.info['vrf']):
						if not self.info['vrf'][vrf]['associations']['address'].get(peer):
							if i == (len(self.info['vrf'])-1) and not found:
								# Last iteration and peer is not found
