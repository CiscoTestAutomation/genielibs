'''
NXOS specific configurations for Mld feature object.
'''
                ...
                def build_config(self, apply=True, attributes=None, unconfig=False,
                    ...
                    with configurations.submode_context(
                        ...
                        if attributes.value('enable'):
                            configurations.append_line('ipv6 pim sparse-mode')
                        ...
                        if attributes.value('immediate_leave'):
                            configurations.append_line('ipv6 mld immediate-leave')

                        # max_groups
                        if attributes.value('max_groups'):
                            configurations.append_line(
                                attributes.format('ipv6 mld state-limit {max_groups}'))
                        ...
                        if attributes.value('robustness_variable'):
                            configurations.append_line(
                                attributes.format('ipv6 mld robustness-variable '
                                                  '{robustness_variable}'))

                        # version
                        if attributes.value('version'):
                            configurations.append_line(
                                attributes.format('ipv6 mld version {version}'))
