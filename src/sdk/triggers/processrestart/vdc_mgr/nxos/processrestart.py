'''
Implementation for VdcMgr Restart triggers
'''
...
class TriggerProcessCrashRestartVdcMgr(TriggerProcessRestart):
    """Restart the running vdc_mgr process(es) with linux command "kill -6 <process>",
    expecting process crashes and generates a core."""
    
    __description__ = """Restart the running vdc_mgr process(es) with linux command "kill -6 <process>",
    expecting process crashes and generates a core.
...
class TriggerProcessKillRestartVdcMgr(TriggerProcessRestart):
    """Restart the running vdc_mgr process(es) with Linux command "kill -9 <process>" """

    __description__ = """Restart the running vdc_mgr process(es) with Linux command "kill -9 <process>"
